...
- 出した 1個原価を焼肉ロス管理アプリの「原価¥」へ反映（下記）

### ロス管理アプリとの連携

「ロス連携」タブから、ここで出した 1個(1皿)あたり原価を `index.html` の「原価¥」に書き込める。
料理名で自動照合し、当たらないものは手で選ぶ。カテゴリーへの新規追加もできる。

- **同じドメインから開いたときだけ**繋がる（localStorage 共有）。
  片方を raw.githack、片方をローカルファイルで開いていると繋がらない
- ロス管理アプリを**一度も開いていない状態では書き込まない**。
  あちらは起動時に `bl_menu_ver` を見てメニューを初期化するため、先に書いても消えるから
- 書き込む前の状態を控えるので「反映前に戻す」で戻せる
- 単価が確定していない材料を含む料理は、既定で送らない。
  金額も 0円 ではなく「未確定」と出す
