...
- 丸一本の歩留まりを1本ごとに記録して、実測を単価マスターに反映
- 納品書の当日単価を貼り付けて取り込み
- 出した 1個原価を焼肉ロス管理アプリの「原価¥」へ反映（下記）

### 歩留まり記録

「歩留まり」タブ。丸一本仕入れ（本鮪など）の歩留まりを**誰が捌いたか・ラウンド重量・
柵の総重量**の3点で1本ごとに残す。卸し手で歩留まりは変わるので、担当者別にも出る。

平均を単価マスターの歩留まりに入れるボタンがある（押したときだけ書き換わる）。
これが入ると、その食材を使う全レシピの原価が推定ではなく実測になる。

可食部がラウンドを超えるなど、記録として成り立たない入力は保存させない。

### 納品書の取り込み

「単価」タブの上部。品名・数量・単価の並びを貼り付けると、単価マスターに当日単価として入る。
タブ / カンマ / スペース区切り、全角数字、`1,400` の桁区切り、`¥`・`円` を吸収する。

数値が3つ以上ある行は `数量 × 単価 ≒ 金額` で単価の位置を確かめる。
**確かめられない行は推測で埋めず、理由付きで確認事項として出す。**
品名がマスターの候補2件以上に当たる場合も自動では当てない。

書き換えるのは仕入単価と更新日だけ。入り数と歩留まりには触らない。

写真しかない場合、ブラウザ内 OCR は日本語の納品書では精度が出ないので載せていない。
写真から表に起こす部分は会話側で行い、その結果を貼り付ける想定。
